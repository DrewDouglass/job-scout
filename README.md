...
- **Real job sources** — Adli's Cowork board connectors (Dice, Indeed, ZipRecruiter via per-user MCP UUIDs) are replaced by **JSearch** (one free RapidAPI key covering Indeed/ZipRecruiter/Glassdoor & more) plus a **LinkedIn ride-along** that reads your own logged-in session, and optional email-alert readers. JSearch runs both location-based and remote-only queries so you see local and remote roles in the same run. A quota-warning banner appears if your monthly free limit is reached.
- **End-to-end** — scoring and resume tailoring run for real (no copy-paste), free on a keyword baseline or via Claude Haiku / a local Ollama model. Keyword scoring gives specific signal-based reasons (e.g. "senior-level title, QA/SDET role") rather than a generic label. Anthropic and JSearch API keys are entered directly in the Settings UI.
